...
- Every user action is one `Operation` in `src/core/ops/registry.ts`. A new op needs, together: a CLI spec (`src/surface/spec.ts`; help and zsh completion derive from it), a view (`src/core/format/views*.ts`), round-trip samples (`test/parity-samples.ts`), a TUI action (`src/tui/actions.ts`), and a leak-test command (`test/leak-commands.ts`). The parity tests fail until all exist.
